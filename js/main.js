...
let buka = ["Halo, Saya RoboChat", "Hai Saya RoboChat dari JogjaHoliday", "Halo namaku RoboChat"];
let bantu = ["Apa yang bisa kubantu?","Kamu perlu bantuan apa?","Apa yang bisa kulakukan untukmu?"];
let tawaran1 = ["Kamu mau tempat wisata seperti apa?", "Tempat wisata jenis apa yang kamu mau?", "Aku memiliki beberapa rekomendasi. Tapi kamu mau yang seperti apa?"];
...
function chatbotvoice(message){
    ...
    if(message.includes('carikan rekomendasi tempat wisata di Jogja' )){
        let finalresult = tawaran1[Math.floor(Math.random() * tawaran1.length)];
        speech.text = finalresult;
    }
    if(message.includes('cari rekomendasi tempat wisata Jogja' )){
        let finalresult = tawaran1[Math.floor(Math.random() * tawaran1.length)];
        speech.text = finalresult;
    }
